                    ...
                    for row in columns_to_process:
                        for col in range(900):
                            ...
                            if img_array[row, col] == 0:
                                # Process based on current array value
                                if array[row, col] == 0:
                                    # 50% chance to change to 1
                                    if random.random() < 0.6:
                                        array[row, col] = 1
                                else:  # Current value is 1
                                    # 100% chance to change to 0
                                    array[row, col] = 0
...
def save_array_to_excel(array, filename="./btsp_result_array.xlsx"):
...
def main():
    ...
    FOLDER1_COLUMNS = 35  # Number of columns to select from the first folder
    FOLDER2_COLUMNS = 35  # Number of columns to select from the second folder
    ...
    result_img.save("./btsp_result_array.png")
